...
struct bst *ans=NULL;
void findsucc(struct bst *b,struct node *p)
{
    equal(p,b->val);
    if(isequal==1)
    {
...
struct bst* finddel(struct bst *b,struct node *p)
{
    if(b==NULL)
        return b;
    equal(p,b->val);
...
int main()
{
    ...
    {
        deletezero(head);
      struct bst *s=finddel(root,head);//deleted node is replaced by its successor
      delnode(s);
      if(head!=NULL)
        mydelete(head);
            head=NULL;
