...
//TODO: WRITE GPU KERNEL. It should not be called repeatedly from the host, but just once. Each time it is called, it may process more 
//than array-element or not process any array-element at all.
...
int main (int argc, char **argv) {
  ...
  for(i=0;i<N;i++) {
    ...
    //deallocating the space (cleanup)
    free(h_C);
